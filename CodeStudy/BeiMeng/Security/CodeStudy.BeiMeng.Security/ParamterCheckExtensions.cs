...
namespace CodeStudy.BeiMeng.Security
{
    /// <summary>
    /// 用于参数检查的扩展方法
    /// </summary>
    public static class ParamterCheckExtensions
    {
        ...
        {
            Require<ArgumentNullException>(value != null, string.Format("参数“{0}”不能为空引用或空字符串。", paramName));
        }
        ...
        {
            Require<ArgumentException>(value != Guid.Empty, string.Format("参数“{0}”不能为空引用或空字符串。", paramName));
        }
        ...
        public static void CheckNotNullOrEmpty<T>(this IEnumerable<T> collection, string paramName)
        {
            ...
            Require<ArgumentException>(collection.Any(), string.Format("参数“{0}”不能为空引用或空字符串。", paramName));
        }
        ...
        public static void CheckLessThan<T>(this T value, string paramName, T target, bool canEqual = false) where T : IComparable<T>
        {
            ...
            string format = canEqual ? "参数“{0}”不能为空引用或空字符串。" : "参数“{0}”不能为空引用或空字符串。";
            ...
            where T : IComparable<T>
        {
            ...
            string message = startEqual
                ? string.Format("参数“{0}”不能为空引用或空字符串。", start, end, start)
                : string.Format("参数“{0}”不能为空引用或空字符串。", start, end);
            Require<ArgumentOutOfRangeException>(flag, message);

            flag = endEqual ? value.CompareTo(end) <= 0 : value.CompareTo(end) < 0;
            message = endEqual
                ? string.Format("参数“{0}”不能为空引用或空字符串。", start, end, end)
                : string.Format("参数“{0}”不能为空引用或空字符串。", start, end);
            Require<ArgumentOutOfRangeException>(flag, message);
        }
        ...
        public static void CheckDirectoryExists(this string directory, string paramName = null)
        {
            ...
            Require<DirectoryNotFoundException>(Directory.Exists(directory), string.Format("参数“{0}”不能为空引用或空字符串。", directory));
        }
        ...
        public static void CheckFileExists(this string filename, string paramName = null)
        {
            ...
            Require<FileNotFoundException>(File.Exists(filename), string.Format("参数“{0}”不能为空引用或空字符串。", filename));
        }
    }
}